...
function frequentVisitors(){
visitsPerDay=$(histogram)
visitors=$(echo "$visitsPerDay" | cut -d ' ' -f 1 | sort | uniq -c)
(echo "$visitors" | awk '{if($1 > 10) print $0}')
}
...
function suspiciousVisitors() {
	suspiciousVisitors=$(tail "$logFile" | egrep -i -f ioc.txt | cut -d ' ' -f 1| sort | uniq -c)
	echo "$suspiciousVisitors"
}
...
do
	...
	else echo "Please input a valid value!"
	fi
done
